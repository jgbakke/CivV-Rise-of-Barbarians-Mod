...
function UpdateDisplay()
	...
	Controls.CivName:SetToolTipString(myCivInfo.Description);
	...
	for iPlayerLoop = 0, GameDefines.MAX_MAJOR_CIVS-1, 1 do
		...
		if (iPlayerLoop ~= g_iPlayer and pOtherPlayer:IsAlive()) then
			...
			if (g_pTeam:IsHasMet(iOtherTeam)) then
				...
				controlTable.CivName:SetToolTipString(civInfo.Description);
...
function InitMinorCivList()
	...
	for iPlayerLoop = GameDefines.MAX_MAJOR_CIVS, GameDefines.MAX_CIV_PLAYERS-1, 1 do
		...
		if( pOtherPlayer:IsMinorCiv() and 
			...
			controlTable.MinorName:SetText( Locale.ConvertTextKey( pOtherPlayer:GetCivilizationDescriptionKey() ) );
			local gov = MapModData.govs[iPlayerLoop]
			if (gov ~= nil) then
				controlTable.MinorName:SetToolTipString(Locale.ConvertTextKey(gov.fullName));
			end
